...
class _CheckoutPageState extends State<CheckoutPage> {
        ...
        if (controller.dataState == DataState.loading) {
          ...
          return SafeArea(
            child: SingleChildScrollView(
              child: defaultWidgetMobile(context),
            ),
          );
        }
      }),
  ...
  Widget ContentSectionMobile(BuildContext context) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const SizedBox(height: 30),
          detailPesananContentMobile(context),
          const SizedBox(height: 60),
          detailTagihanContentMobile(),
        ],
      ),
    );
  }
  ...
  Widget detailPesananContentMobile(BuildContext context) {
    final cartController = context.watch<CartController>();
    final carts = cartController.carts
        .where((cart) =>
            cart['user']['id'] == user['id'] &&
  ...
  Widget detailTagihanContentMobile() {
    final wilayahController = context.watch<WilayahController>();
    final checkoutController = context.watch<CheckoutController>();
    final cartController = context.watch<CartController>();
